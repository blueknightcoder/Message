...
class MessagingApp:
    ...
    def _handle_message(self, msg):
        ...
        elif action == "invalid_members":
            # Some members couldn't be added when creating chat
            invalid = msg.get("invalid_members", [])
            if invalid:
                self.root.after(0, lambda: messagebox.showwarning(
                    "Some users not added",
                    f"These users are not online and weren't added to the chat:\n\n{', '.join(invalid)}\n\nThey can be added later when they're online."
                ))
        
        elif action == "add_member_failed":
            # Failed to add a member
            error_msg = msg.get("message", "Could not add user")
            self.root.after(0, lambda: messagebox.showerror("Could not add user", error_msg))
